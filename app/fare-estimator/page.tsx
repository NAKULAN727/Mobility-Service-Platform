...
export default function FareEstimatorPage() {
  ...
  const [mode, setMode] = useState<"distance" | "route">("distance");

  // State for Distance-based Estimator (Member 2)
  ...
  // State for Route-based Estimator (Member 3)
  const [pickup, setPickup] = useState("");
  const [destination, setDestination] = useState("");
  const [rideType, setRideType] = useState("Standard");
  const [trafficLevel, setTrafficLevel] = useState<"Normal" | "Moderate" | "High">("Normal");
  const [routeResult, setRouteResult] = useState<FareResult | null>(null);
  ...
  }, [vehicleType, bookingType, distance, mode]);
  ...
  const handleCalculateRoute = async () => {
    ...
    try {
      ...
      setRouteResult(fareResult);
    } catch (err: any) {
      console.error("Error calculating fare:", err);
      setRouteError(err.message || "Failed to calculate fare. Please try again.");
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      <div className="pt-32 px-6 max-w-4xl mx-auto flex flex-col gap-8">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 leading-relaxed">Get an instant fare estimate before you book. Prices are transparent and fixed.</p>
        </div>
        ...
        <div className="flex gap-6 border-b border-slate-200 pb-2">
          ...
          >
            Interactive Route Estimator (with AI)
          </button>
        </div>
        ...
        {mode === "distance" ? (
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <button
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm py-4 rounded-xl cursor-pointer transition-all duration-200 shadow-md shadow-emerald-600/10 hover:shadow-emerald-600/20 active:scale-[0.99] border-none"
                    >
                      Proceed to Booking →
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="premium-glass-card p-6 shadow-md border border-slate-200/50 bg-white">
              <h2 className="text-xl font-heading font-black text-slate-800 mb-6">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-700 focus:ring-emerald-600 focus:border-emerald-600 outline-none"
                  >
                ...
                <div>
                  ...
                  <select 
                    ...
                    disabled={!pickup}
                    className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-700 focus:ring-emerald-600 focus:border-emerald-600 disabled:bg-gray-100 disabled:text-gray-400 outline-none"
                  >
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Ride Type</label>
                  <select 
                    value={rideType} 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Traffic Level</label>
                  <select 
                    value={trafficLevel} 
                ...
                <button
                  ...
                  disabled={routeLoading || !pickup || !destination}
                  className="w-full bg-emerald-600 text-white font-black py-4 rounded-xl hover:bg-emerald-700 focus:outline-none disabled:bg-slate-300 transition-colors cursor-pointer border-none shadow-md mt-4"
                >
                  {routeLoading ? (
            ...
            <div className="flex flex-col gap-5 sticky top-32">
              {routeResult ? (
                <div className="w-full">
                  <FareCard result={routeResult} rideType={rideType} trafficLevel={trafficLevel} />
                </div>
              ) : (
                <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl bg-white/50 text-slate-400 p-8 text-center">
